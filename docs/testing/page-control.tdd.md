...
## Actualización del contrato de tamaño de fuente (2026-09-10)

El umbral original de «menos de 2 KB» sobre `src/scripts/page-control.ts` se fijó en el commit RED `35d8249` **antes de que existiera ninguna implementación** (el archivo pesaba 0 bytes en ese momento): fue un proxy de diseño para forzar «sin dependencias externas», no una medición de rendimiento real. Funcionalidad legítima añadida después (efecto de pila/parallax y arrastre por ratón en la vista de Proyectos) elevó el archivo a ~4,09 KB sin introducir ninguna dependencia npm; el script inline resultante en producción sigue pesando solo ~1 KB minificado y comprimido con gzip. El contrato de test se actualizó para proteger la invariante real —cero imports de paquetes npm externos en `page-control.ts`— en lugar de un tamaño de fuente arbitrario. De forma relacionada, la aserción sobre el color `#22d3ee` se actualizó a `var(--color-primary)`, reflejando la tokenización semántica de color introducida en el commit `c587304`; el valor cian (`#22d3ee` en `tokens.css`) y su uso en el estado activo/foco de `pageControl__tab` no cambiaron.
