...
**Static-first sin backend.** El formulario de contacto compone un URI `mailto:` en el navegador y lo entrega al cliente de correo del usuario — `window.location.assign(mailto)`. Ningún mensaje toca jamás la infraestructura del proyecto: no hay endpoint de formulario que limitar por tasa, ni base de datos de envíos que asegurar, ni acuerdo de encargado de tratamiento para un servicio de formularios de terceros. Esto no significa ausencia de obligaciones RGPD sobre la correspondencia: el correo transita por el proveedor del remitente y llega al buzón corporativo alojado en Zoho Mail, donde sí aplican el contrato de encargo del tratamiento (art. 28 RGPD) y los derechos de la persona interesada sobre el mensaje almacenado — véase `docs/PROCESSOR_DPA_REGISTER.md` y `docs/PRIVACY_OPERATIONS.md`.
